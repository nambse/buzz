...
before(() => Object.assign(globalThis, {
  window: dom.window,
...
const company = { id: "selected", name: "Example company", relayUrl: "ws://localhost:3038", pubkey: "a".repeat(64), addedAt: "2026-09-06" };
const other = { ...company, id: "other", name: "Retained other", relayUrl: "wss://other.example" };
...
test("private company provider refuses add/switch/onboarding while reconnect and identity metadata remain usable", async () => {
  ...
  const { CommunitiesProvider, useCommunities } = await import("@/features/communities/useCommunities.tsx");
  const { CommunityOnboardingProvider, useCommunityOnboarding } = await import("@/features/onboarding/communityOnboarding.tsx");
  let state;
  let onboarding;
  function Probe() {
  ...
  render(React.createElement(CommunitiesProvider, null,
    React.createElement(CommunityOnboardingProvider, { enabled: true }, React.createElement(Probe))));
  ...
  assert.equal(onboarding.start({ relayUrl: other.relayUrl, source: "deep-link-join" }), false);
  assert.equal(localStorage.getItem("buzz-community-onboarding-transaction.v1"), pending);
  await act(async () => state.switchCommunity(company.id));
  await act(async () => state.reconnectCommunity());
  assert.equal(state.reinitKey, 1);
  await act(async () => state.updateCommunity(company.id, { pubkey: "b".repeat(64) }));
  assert.equal(state.activeCommunity.pubkey, "b".repeat(64));
  assert.deepEqual(JSON.parse(localStorage.getItem("buzz-communities"))[0], other);
});
...
test("fixed first-company bootstrap preserves older saved entries", async () => {
  ...
  const { initFirstCommunity } = await import("@/features/communities/communityStorage.ts");
  const created = initFirstCommunity(company.relayUrl, company.pubkey, company.name);
  assert(created);
  assert.equal(localStorage.getItem("buzz-active-community-id"), created.id);
  assert.deepEqual(JSON.parse(localStorage.getItem("buzz-communities")), [other, created]);
});
...
test("private membership recovery keeps retry and key import without community or invite actions", async () => {
  ...
  const { CommunityOnboardingProvider } = await import("@/features/onboarding/communityOnboarding.tsx");
  const { MembershipDenied } = await import("@/features/onboarding/ui/MembershipDenied.tsx");
  let retries = 0;
  let imported = null;
  const view = render(React.createElement(CommunityOnboardingProvider, null,
    React.createElement(MembershipDenied, {
      ...
      onRetry: () => { retries += 1; },
      onImportKey: async (value) => { imported = value; },
    })));
  ...
  fireEvent.change(view.getByLabelText("Private key"), { target: { value: synthetic } });
  await act(async () => fireEvent.click(view.getByRole("button", { name: "Import key" })));
