...
async fn employee_runtime_v5_read_state_replacement_preserves_use_but_source_edit_revokes() {
    ...
    let generation: i64 = sqlx::query_scalar(
        "SELECT generation FROM office_authority_generations WHERE company_id=$1",
    )
    .bind(c.x.f.company)
    .fetch_one(&c.x.f.pool)
    .await
    ...
    assert!(
        db.replace_parameterized_event(community, &replacement, &d_tag, None)
            .await
            .unwrap()
            .1
    );
    ...
    assert_eq!(
        epoch(c).await,
        before,
        "unrelated read-state deletion must not retire the selected employee source"
    );
    let changed_generation: i64 = sqlx::query_scalar(
        "SELECT generation FROM office_authority_generations WHERE company_id=$1",
    )
    .bind(c.x.f.company)
    .fetch_one(&c.x.f.pool)
    ...
    assert!(
        ...
        "the existing global Office mutation fence is still active"
    );
    assert!(c.current(run).await);
    let report =
    ...
    for (kind, expected_epoch) in [(30078, before + 1), (9, before + 2)] {
        let changed = sqlx::query("UPDATE events SET kind=$3 WHERE community_id=$1 AND id=$2")
            .bind(c.x.f.community)
            .bind(hex::decode(&c.x.source).unwrap())
            .bind(kind)
            .execute(&mut *tx)
            .await
            .unwrap();
        ...
        assert_eq!(
            ...
            "transition into or out of plaintext must retire the epoch"
        );
    }
