...
impl EncryptedFixture {
    async fn new() -> Self {
        ...
        employee.office.signer_ref = CredentialRef::parse("credential://synthetic/encrypted-authority-office").unwrap();
        ...
        let channel = direct_support::create(&f.pool, f.community, &[f.operator.public_key().to_bytes(), *employee_key.as_bytes()]).await;
        let scope = f.control.resolve_company_for_community(f.community).await.unwrap();
        direct_support::select(&f.control, &scope, channel, &employee.id).await;
        let pair = ConfiguredDmPair {
            selection_id: Uuid::new_v4(), channel_id: channel, employee_id: employee.id,
            human_public_key: OfficePublicKey::parse_hex(&f.operator.public_key().to_hex()).unwrap(),
            employee_public_key: employee_key, office_binding_id: binding, key_version: 0, decrypt_ref: employee.office.signer_ref,
        };
        ...
        Self { f, scope, channel, pair, jobs, app }
    }
    ...
    fn path(&self) -> String { format!("/api/v1/channels/{}/encrypted-dm/authority", self.channel) }
    async fn read(&self) -> (StatusCode, Value) {
        bounded(&self.app, signed(&self.f.operator, "GET", &self.path(), "", false)).await
    }
...
fn observation(x: &EncryptedFixture, value: &Value) -> i64 {
    let mut expected = vec!["format","company_id","community_id","channel_id","employee_id","human_public_key","employee_public_key","pair_hash","selection_id","selection_generation","office_binding_id","key_version","office_generation","authority_epoch","observed_at","valid_before"];
    expected.sort();
    let mut actual = value.as_object().unwrap().keys().map(String::as_str).collect::<Vec<_>>(); actual.sort();
    ...
    assert_eq!(value["human_public_key"], x.f.operator.public_key().to_hex());
    assert_eq!(value["employee_public_key"], hex::encode(x.pair.employee_public_key.as_bytes()));
    assert_eq!(value["office_binding_id"], x.pair.office_binding_id.to_string());
    assert_eq!(value["selection_id"], x.pair.selection_id.to_string());
    assert_eq!(value["key_version"], "0");
    let keys = [x.f.operator.public_key().to_bytes(), *x.pair.employee_public_key.as_bytes()];
    let refs = keys.iter().map(|key| key.as_slice()).collect::<Vec<_>>();
    assert_eq!(value["pair_hash"], hex::encode(buzz_db::dm::compute_participant_hash(&refs)));
    for key in ["selection_generation","key_version","office_generation","authority_epoch"] {
    ...
    let observed = chrono::DateTime::parse_from_rfc3339(value["observed_at"].as_str().unwrap()).unwrap();
    let until = chrono::DateTime::parse_from_rfc3339(value["valid_before"].as_str().unwrap()).unwrap();
...
async fn encrypted_dm_authority_signed_native_dto_requires_both_grants_and_exact_human_host() {
    ...
    for (actor, channel, employee) in [(&x.f.operator, x.f.channel, "cem"), (&x.f.operator, x.channel, "unselected-employee"), (&x.f.reader, x.channel, "cem")] {
    ...
    wrong_host.headers_mut().insert("host", "foreign.example".parse().unwrap());
    assert_eq!(bounded(&x.app, wrong_host).await.0, StatusCode::UNAUTHORIZED);
    ...
    assert_eq!(bounded(&app, signed(&foreign.operator, "GET", &x.path(), "", false)).await.0, StatusCode::NOT_FOUND);
...
async fn encrypted_dm_authority_disable_and_member_restore_require_new_current_generations() {
    let x = EncryptedFixture::new().await;
    let (status, before) = x.read().await; assert_eq!(status, StatusCode::OK, "{before}");
    let epoch = observation(&x, &before);
    let disabled = x.jobs.set_enabled(&x.scope, x.pair.selection_id, 1, false).await.unwrap();
    assert_eq!(disabled, 2);
    assert_eq!(x.read().await, (StatusCode::NOT_FOUND, json!({"error":{"code":"not_found"}})));
    let enabled = x.jobs.set_enabled(&x.scope, x.pair.selection_id, disabled, true).await.unwrap();
    assert_eq!(enabled, 3);
    let (status, reopened) = x.read().await; assert_eq!(status, StatusCode::OK, "{reopened}");
    let enabled_epoch = observation(&x, &reopened); assert!(enabled_epoch > epoch);
    assert_eq!(reopened["selection_generation"], "3");
    x.member(false).await;
    assert_eq!(x.read().await, (StatusCode::NOT_FOUND, json!({"error":{"code":"not_found"}})));
    x.member(true).await;
    let (status, restored) = x.read().await; assert_eq!(status, StatusCode::OK, "{restored}");
    assert!(observation(&x, &restored) > enabled_epoch);
    assert_eq!(restored["selection_generation"], "3");
    for field in ["selection_id","office_binding_id","pair_hash","key_version","human_public_key","employee_public_key"] {
