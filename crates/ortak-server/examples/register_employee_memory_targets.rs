...
use ortak_memory::{HonchoDeploymentSelection, HonchoEmployeeBinding, HonchoMemoryAdapter,
    HonchoMemoryConfig, ResolvedHonchoToken, HONCHO_VERSION, PROTOCOL};
...
enum Action { Plan, Register, Readback, Recover }
...
fn arguments() -> Result<(PathBuf, Action)> {
    ...
    let action = if args.len() == 2 { Action::Plan } else {
        if args[2] != "--action" { return Err("usage_config_then_optional_action"); }
        match args[3].to_str() {
            Some("plan") => Action::Plan, Some("register") => Action::Register,
            Some("readback") => Action::Readback, Some("recover") => Action::Recover,
...
fn emit(value: Value) -> Result<()> {
    ...
    if value.len() > 4096 { return Err("output_bound"); }
    println!("{value}");
    Ok(())
}
...
fn adapter(config: &Config, target: &Target, scope: &CompanyScope) -> Result<HonchoMemoryAdapter> {
    ...
    HonchoMemoryAdapter::new(scope, HonchoMemoryConfig {
        deployment: HonchoDeploymentSelection {
            deployment_id:d.deployment_id, protocol:PROTOCOL.into(), honcho_version:HONCHO_VERSION.into(),
            endpoint_ref:d.endpoint_ref.clone(), origin:d.origin.clone(), token_ref:d.token_ref.clone(),
        },
        employees:vec![HonchoEmployeeBinding {
            employee_id:target.original.employee_id.clone(), binding:target.original.binding.clone(),
            mode:ProvisioningMode::Adopt, allow_company_truth:false, allowed_projects:BTreeSet::new(),
        }],
        request_timeout:Duration::from_secs(3), witness_lifetime:Duration::from_secs(60),
    }, token).map_err(|_| "adapter_selection")
}

async fn one(control: &PgControlPlane, scope: &CompanyScope, config: &Config,
    target: &Target, action: Action) -> Result<Value>
{
    if let Some(retained) = readback::existing(control.pool(), config, target).await? {
        return Ok(retained);
    }
    if action == Action::Readback {
        return Ok(json!({"status":"not_registered","employee_id":target.original.employee_id,
    ...
    let namespace = adapter.inspect_reviewed_employee_namespace(&target.original).await
        .map_err(|_| "original_namespace_inspection")?;
    if action == Action::Recover {
        let receipt = adapter.recover_employee_namespace_diagnostic(&namespace, &target.diagnostic).await
            .map_err(|_| "diagnostic_cleanup_unconfirmed")?;
        return Ok(json!({"status":"diagnostic_cleaned_no_target","employee_id":target.original.employee_id,
            "operation_id":receipt.operation_id,"erased":receipt.erased,
            "cleanup_receipt":receipt,"target_registered":false}));
    }
    let witness = adapter.validate_reviewed_employee_namespace(&namespace, &target.diagnostic).await
    ...
    for attempt in 0..2 {
        let result = ortak_server::employee_memory_exports::register_target(
            control, scope, &adapter, &witness, target.destination_channel_id, target.valid_until).await;
        if result.is_ok() {
            return readback::existing(control.pool(), config, target).await?.ok_or("target_readback_missing");
        }
...
async fn run() -> Result<()> {
    ...
    emit(json!({"format":"ortak-employee-target-operator-receipt/1","config_sha256":fingerprint,
    ...
    if action == Action::Plan { return Ok(()); }
    let database = config.database()?;
    let pool = ortak_server::connect_private_database(&database).await.map_err(|_| "database_connection")?;
    drop(database);
    let control = PgControlPlane::new(pool);
    let scope = control.resolve_company_for_community(config.community_id).await.map_err(|_| "company_selection")?;
    if scope.company_id() != config.company_id { return Err("company_selection"); }
    for target in &config.targets {
        match one(&control, &scope, &config, target, action).await {
            Ok(value) => emit(value)?,
            Err(code) => {
                emit(json!({"status":"unresolved","employee_id":target.original.employee_id,
...
async fn main() {
    let result = match ortak_server::shutdown::Shutdown::install() {
        Ok(mut shutdown) => shutdown.until(tokio::time::timeout(Duration::from_secs(300), run())).await,
        Err(_) => { eprintln!("employee-target-operator: shutdown_registration"); std::process::exit(1); }
    };
    if let Ok(ortak_server::shutdown::Outcome::Completed(Ok(Ok(())))) = result { return; }
