...
pub fn read(path: &Path) -> Result<(Config, String)> {
    ...
    if raw.len() as u64 != before.st_size as u64
        || stamp(&initial) != stamp(&after)
        || stamp(&initial) != stamp(&linked)
        || !linked.is_file()
...
impl Config {
    fn validate(&self) -> Result<()> {
        ...
        if self.format != "ortak-employee-target-operator/1"
            || self.company_id.is_nil()
            || self.community_id.is_nil()
            || d.deployment_id.is_nil()
            || !matches!(self.database_port, 55432 | 55433)
            || !env_name(&self.database_env)
            || !env_name(&d.token_env)
        ...
        for item in &self.targets {
            ...
            if r.company_id != self.company_id
                || r.deployment_id != d.deployment_id
                || r.binding.endpoint_ref != d.endpoint_ref
                || r.binding.adapter != "honcho"
                || item.destination_channel_id.is_nil()
                || diagnostic.operation_id.is_nil()
                || diagnostic.employee_revision_id.is_nil()
                || diagnostic.employee_lifecycle_epoch < 0
