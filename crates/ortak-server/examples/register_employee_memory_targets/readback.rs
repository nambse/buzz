...
use super::{config::{self, Config, Target}, Result};
...
pub async fn existing(pool: &PgPool, config: &Config, target: &Target) -> Result<Option<Value>> {
    let rows = sqlx::query("SELECT id,community_id,binding,creation_receipt,registration_receipt,
        ...
        .bind(target.destination_channel_id).bind(config.deployment.deployment_id)
        .fetch_all(pool).await.map_err(|_| "target_readback")?;
    if rows.is_empty() { return Ok(None); }
    if rows.len() != 1 { return Err("target_ambiguous"); }
    let row = &rows[0];
    let namespace_hash = hex::encode(row.try_get::<Vec<u8>, _>("namespace_hash").map_err(|_| "target_metadata")?);
    let mut original: Value = row.try_get("creation_receipt").map_err(|_| "target_metadata")?;
    ...
    if object.remove("protocol") != Some(json!(REVIEWED_EMPLOYEE_PROTOCOL))
        ...
        || row.try_get::<Uuid, _>("community_id").map_err(|_| "target_metadata")? != config.community_id
        || row.try_get::<Value, _>("binding").map_err(|_| "target_metadata")?
            != serde_json::to_value(&target.original.binding).map_err(|_| "target_metadata")?
        || row.try_get::<DateTime<Utc>, _>("valid_until").map_err(|_| "target_metadata")? != target.valid_until
    {
        return Err("target_identity_conflict");
    }
    let registration: Value = row.try_get("registration_receipt").map_err(|_| "target_metadata")?;
    let receipt: EmployeeNamespaceDiagnosticReceipt = serde_json::from_value(
        registration.get("diagnostic").cloned().ok_or("target_metadata")?
    ...
    if registration.get("format") != Some(&json!("ortak-employee-namespace-registration/1"))
        ...
        || !receipt.erased || receipt.write_request_hash.is_none()
    {
        return Err("target_diagnostic_conflict");
    }
    Ok(Some(json!({"status":"registered_retained","target_id":row.try_get::<Uuid,_>("id").map_err(|_| "target_metadata")?,
...
pub async fn current_employee(pool: &PgPool, config: &Config, target: &Target) -> Result<()> {
    ...
    if row.try_get::<Uuid,_>("active_revision_id").map_err(|_| "employee_read")? != target.diagnostic.employee_revision_id
        || row.try_get::<i64,_>("lifecycle_epoch").map_err(|_| "employee_read")? != target.diagnostic.employee_lifecycle_epoch
        || row.try_get::<Value,_>("memory").map_err(|_| "employee_read")?
