...
use ortak_office::{encrypted::{decode,DmDecryptKey,jobs::{ConfiguredDmPair,DmDecryptClaim,DmOuterSource,PgDecryptJobs},key_provider::{DmOfficeKeyBinding,EnvDmKeyProvider,OfficeKeyPurpose}},transport::OfficeSignerBinding};
...
struct EncryptedFixture { f:Fixture,human:Keys,employee:Keys,channel:Uuid,pair:ConfiguredDmPair,jobs:PgDecryptJobs,protected:PgConfidentialRuns }
impl EncryptedFixture {
    async fn new()->Self {
        ...
        assert_eq!(std::env::var("ORTAK_TEST_CONFIDENTIAL_SYNTHETIC_KEY").unwrap(),"31".repeat(32));
        ...
        let channel=direct_support::create(&f.pool,f.community_id,&[human.public_key().to_bytes(),employee.public_key().to_bytes()]).await;
        ...
        let pair=ConfiguredDmPair {selection_id:Uuid::new_v4(),channel_id:channel,employee_id:f.employee.id.clone(),
            human_public_key:OfficePublicKey::parse_hex(&human.public_key().to_hex()).unwrap(),employee_public_key:OfficePublicKey::parse_hex(&employee.public_key().to_hex()).unwrap(),
            office_binding_id:binding,key_version:0,decrypt_ref:f.employee.office.signer_ref.clone()};
        ...
        Self {f,human,employee,channel,pair,jobs,protected}
    }
    fn rumor(&self)->UnsignedEvent {
        let mut rumor=EventBuilder::private_msg_rumor(self.employee.public_key(),"Protected admission canary\nİ 🧭")
            .build(self.human.public_key());
        rumor.ensure_id(); rumor
    }
    async fn outer(&self,rumor:UnsignedEvent)->Event {
        EventBuilder::gift_wrap(&self.human,&self.employee.public_key(),rumor,[]).await.unwrap()
    }
    async fn accept(&self,outer:&Event)->DmDecryptClaim {
        let at=Utc.timestamp_opt(outer.created_at.as_secs() as i64,0).single().unwrap();
        ...
        self.f.control.insert_accepted_event(self.f.community_id,&InboxEvent {event_id:MessageId::from_bytes(outer.id.to_bytes()),event_created_at:at,event_kind:1059,author_pubkey:outer.pubkey.to_bytes(),channel_id:None}).await.unwrap();
        assert!(self.jobs.enqueue(&self.f.scope,self.pair.selection_id,&DmOuterSource::new(outer.id,at).unwrap()).await.unwrap());
        let claim=self.jobs.claim_next(&self.f.scope,Uuid::new_v4()).await.unwrap().unwrap();
        assert!(self.jobs.claim_is_current(&self.f.scope,&claim).await.unwrap());
        let verified=decode(&DmDecryptKey::for_recipient(&self.employee,self.employee.public_key()).unwrap(),claim.expected(),claim.outer_bytes()).unwrap();
        self.jobs.record_verified(&self.f.scope,&claim,&verified).await.unwrap();
        claim
    }
    async fn prepare(&self,claim:&DmDecryptClaim)->ProtectedConfidentialRun {
        let verified=decode(&DmDecryptKey::for_recipient(&self.employee,self.employee.public_key()).unwrap(),claim.expected(),claim.outer_bytes()).unwrap();
        let prepared=self.protected.prepare(&self.f.scope,claim,&verified).await.unwrap();
        let provider=EnvDmKeyProvider::new(vec![DmOfficeKeyBinding {
            signer:OfficeSignerBinding {company_id:self.f.scope.company_id(),employee_id:self.f.employee.id.clone(),signer_ref:self.pair.decrypt_ref.clone(),public_key:self.pair.employee_public_key,secret_env:"ORTAK_TEST_CONFIDENTIAL_SYNTHETIC_KEY".into()},
            office_binding_id:self.pair.office_binding_id,key_version:0,purposes:vec![OfficeKeyPurpose::WrapMaster,OfficeKeyPurpose::UnwrapMaster],
        }]).unwrap();
...
async fn confidential_atomic_ciphertext_admission_dedupes_wrappers_and_quarantines_ordinary_paths(){
    ...
    let receipt=x.protected.commit(&x.f.scope,&claim,&protected).await.unwrap();
    assert!(!receipt.duplicate_rumor);
    assert_eq!(x.protected.commit(&x.f.scope,&claim,&protected).await.unwrap(),receipt);
    let second=x.outer(rumor).await;
    let claim2=x.accept(&second).await;
    let protected2=x.prepare(&claim2).await;
    let duplicate=x.protected.commit(&x.f.scope,&claim2,&protected2).await.unwrap();
    assert!(duplicate.duplicate_rumor); assert_eq!(duplicate.run_id,receipt.run_id);
    ...
    assert!(x.jobs.claim_next(&x.f.scope,Uuid::new_v4()).await.unwrap().is_none());
    ...
    assert!(PgConfidentialRuns::load_current_on(&mut tx,&x.f.scope,receipt.run_id).await.unwrap().is_some());
...
async fn confidential_revoke_restore_never_revives_content_but_commit_receipt_and_cancel_survive(){
    ...
    let mut tx=x.f.pool.begin().await.unwrap();
    assert!(PgConfidentialRuns::load_current_on(&mut tx,&x.f.scope,receipt.run_id).await.unwrap().is_none());
    tx.rollback().await.unwrap();
    assert_eq!(x.protected.commit(&x.f.scope,&claim,&protected).await.unwrap(),receipt);
    assert!(x.protected.cancel(&x.f.scope,receipt.run_id).await.unwrap());
...
async fn confidential_revoke_after_protection_rolls_back_every_admission_effect(){
    ...
    let protected=x.prepare(&claim).await;
    x.remove_human(true).await;
    assert!(x.protected.commit(&x.f.scope,&claim,&protected).await.is_err());
