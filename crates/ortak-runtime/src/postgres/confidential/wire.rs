...
struct Binding<'a> {
    adapter: &'a str,
    credential_refs: &'a [ortak_domain::CredentialRef],
    model: &'a str,
    options: &'a std::collections::BTreeMap<String, String>,
    profile_ref: &'a Option<String>,
    workspace_ref: &'a str,
}
#[derive(Serialize)]
struct Spec<'a> {
...
pub(super) fn snapshot(
    ...
    let identity =
        serde_json::from_slice(identity.canonical_bytes()).map_err(|_| Error::Payload)?;
    let inner = Inner {
        format: "ortak-confidential-run/1",
        identity,
        spec: Spec {
            binding: Binding {
