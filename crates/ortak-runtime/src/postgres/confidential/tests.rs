...
fn identity()->ValidatedIdentity {
    let vector:Value=serde_json::from_str(include_str!("../../../../ortak-control/src/confidential/vector.json")).unwrap();
    ValidatedIdentity::parse(vector["expected"]["identity_utf8"].as_str().unwrap().as_bytes()).unwrap()
}
...
fn confidential_inner_uses_canonical_reduced_context_and_exact_utf8(){
    ...
    let encoded=wire::snapshot(&identity(),&binding(),run,run,"fixture","confidential_run:fixture","pair",None,text).unwrap();
    ...
    assert_eq!(wire["spec"]["context"],json!({"conversation_ref":"pair","reply_to_message_id":null}));
    assert_eq!(wire["spec"]["permissions"],json!({"allowed_networks":[],"allowed_tools":[],"allowed_workspaces":[],"approval_required":[]}));
...
fn confidential_inner_rejects_content_bound_and_nul_without_truncating(){
    ...
    for text in [String::new(),"x".repeat(8193),"nul\0content".into()] {
        assert!(wire::snapshot(&identity(),&binding(),run,run,"fixture","key","pair",None,&text).is_err());
    }
    let input="İ".repeat(4096);
    let encoded=wire::snapshot(&identity(),&binding(),run,run,"fixture","key","pair",None,&input).unwrap();
    assert_eq!(serde_json::from_slice::<Value>(&encoded).unwrap()["spec"]["input"],input);
}
