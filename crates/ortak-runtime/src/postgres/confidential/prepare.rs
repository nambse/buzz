use chrono::Utc;
use ortak_control::{
    confidential::{PayloadPurpose, ValidatedIdentity},
    CompanyScope,
};
use ortak_domain::{Employee, PermissionPolicy};
use ortak_office::encrypted::{
    jobs::{DmDecryptClaim, PgDecryptJobs},
    key_provider::{DmKeySelection, EnvDmKeyProvider},
    VerifiedDmRumor,
};
use sqlx::Row;
use uuid::Uuid;
...
use super::{
    wire, ConfidentialAdmissionError as Error, PgConfidentialRuns, PreparedConfidentialRun,
    ProtectedConfidentialRun, Result,
};
...
impl PgConfidentialRuns {
    ...
    pub async fn prepare(
        ...
        verified: &VerifiedDmRumor,
    ) -> Result<PreparedConfidentialRun> {
        ...
        if row.try_get::<Vec<u8>, _>("seal_id")? != verified.seal_id().to_bytes()
            || row.try_get::<chrono::DateTime<Utc>, _>("seal_created_at")?
                != verified.seal_created_at()
            || row.try_get::<Vec<u8>, _>("rumor_id")? != verified.rumor_id().to_bytes()
            || row.try_get::<chrono::DateTime<Utc>, _>("rumor_created_at")?
                != verified.rumor_created_at()
            ...
            || row.try_get::<Option<Vec<u8>>, _>("reply_to")?
                != verified.reply_to().map(|id| id.to_bytes().to_vec())
            || verified.source() != claim.expected()
        {
            return Err(Error::Refused);
        }
        let employee: Employee =
            serde_json::from_value(row.try_get("manifest")?).map_err(|_| Error::Refused)?;
        employee.validate_definition().map_err(|_| Error::Refused)?;
        let binding = serde_json::from_value(row.try_get::<serde_json::Value, _>("binding")?)
            .map_err(|_| Error::Refused)?;
        if employee.runtime != binding || employee.permissions != PermissionPolicy::default() {
            return Err(Error::Refused);
        }
        ...
        let identity: Vec<u8> =
            sqlx::query_scalar("SELECT ortak_confidential_dm_identity($1,$2,$3,$4)")
                .bind(scope.company_id())
                .bind(&source)
                .bind(run)
                .bind(key_id)
        ...
        tx.commit().await?;
        Ok(PreparedConfidentialRun {
            identity,
            plaintext,
            source_id: source,
            run_id: run,
            key_id,
            signer_ref: claim.identity().decrypt_ref.clone(),
...
impl PreparedConfidentialRun {
    ...
    pub fn protect(mut self, provider: &EnvDmKeyProvider) -> Result<ProtectedConfidentialRun> {
        ...
        let selected =
            DmKeySelection::from_expected_claims(&self.identity, self.signer_ref.clone());
        let wrapped = provider
            .wrap_master(&selected, &master)
            .map_err(|error| match error {
                ortak_office::encrypted::key_provider::DmKeyError::Unavailable => {
                    Error::Unavailable
                }
                _ => Error::Refused,
            })?;
        let master = crate::confidential::ConfidentialMasterKey::from_owned(master);
        let snapshot = crate::confidential::seal(
            &master,
            &self.identity,
