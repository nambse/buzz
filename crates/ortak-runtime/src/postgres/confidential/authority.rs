...
impl PgConfidentialRuns {
    ...
    pub async fn authority(&self,scope:&CompanyScope,channel:Uuid,human:&[u8;32])->Result<Option<EncryptedDmAuthority>>{
        let Some(community)=scope.community_id() else { return Ok(None); };
        if channel.is_nil() { return Ok(None); }
        ...
        if rows.is_empty() { tx.commit().await?; return Ok(None); }
        if rows.len()!=1 { return Err(Error::Refused); }
        let row=&rows[0];
        let observed_at:DateTime<Utc>=row.try_get("observed_at")?;
        let valid_before:DateTime<Utc>=row.try_get("valid_before")?;
        let result=EncryptedDmAuthority {format:"ortak-native-encrypted-dm-authority/1",company_id:scope.company_id(),community_id:community,channel_id:channel,
            human_public_key:hex::encode(human),employee_id:row.try_get("employee_id")?,
            employee_public_key:hex::encode(row.try_get::<Vec<u8>,_>("employee_public_key")?),pair_hash:row.try_get("pair_hash")?,
            selection_id:row.try_get("selection_id")?,selection_generation:row.try_get::<i64,_>("generation")?.to_string(),
            office_generation:office.generation().to_string(),authority_epoch:office.generation().to_string(),
            key_version:row.try_get::<i64,_>("key_version")?.to_string(),office_binding_id:row.try_get("office_binding_id")?,
            observed_at,valid_before:valid_before.min(observed_at+chrono::Duration::seconds(5))};
        if result.valid_before<=result.observed_at { return Err(Error::Refused); }
        tx.commit().await?;
        Ok(Some(result))
    }
}
