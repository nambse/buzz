use ortak_control::{
    confidential::{ConfidentialEnvelope, PayloadPurpose, ValidatedIdentity},
...
use ortak_office::encrypted::{
    ...
    key_provider::WrappedMasterKey,
};
use sqlx::{PgConnection, Row};
use uuid::Uuid;

use super::{
    ConfidentialAdmissionError as Error, ConfidentialAdmissionReceipt, CurrentConfidentialPayload,
    PgConfidentialRuns, ProtectedConfidentialRun, Result,
};
...
impl PgConfidentialRuns {
    ...
    pub async fn commit(
    ...
    ) -> Result<ConfidentialAdmissionReceipt> {
        ...
        if !PgDecryptJobs::retained_claim_matches_on(&mut tx, scope, claim).await? {
            return Err(Error::Refused);
        }
        ...
        let receipt = ConfidentialAdmissionReceipt {
            run_id: row.try_get("committed_run_id")?,
            duplicate_rumor: row.try_get("duplicate_rumor")?,
        };
        tx.commit().await?;
        Ok(receipt)
    }
    ...
    pub async fn load_current_on(
        connection: &mut PgConnection,
        scope: &CompanyScope,
        run: Uuid,
    ) -> Result<Option<CurrentConfidentialPayload>> {
        ...
        if expected != row.try_get::<Vec<u8>, _>("identity_bytes")? {
            return Err(Error::Payload);
        }
        let identity = ValidatedIdentity::parse(&expected).map_err(|_| Error::Payload)?;
        let snapshot = ConfidentialEnvelope::parse(&row.try_get::<Vec<u8>, _>("envelope_bytes")?)
            .map_err(|_| Error::Payload)?;
        snapshot
            .header()
            .require_expected(&identity, PayloadPurpose::Snapshot, 0)
            .map_err(|_| Error::Payload)?;
        let wrapped = WrappedMasterKey::parse(&row.try_get::<Vec<u8>, _>("wrapped_key")?)
            .map_err(|_| Error::Payload)?;
        let signer_ref = CredentialRef::parse(row.try_get::<String, _>("decrypt_ref")?)
            .map_err(|_| Error::Payload)?;
        Ok(Some(CurrentConfidentialPayload {
            identity,
            snapshot,
            wrapped,
            signer_ref,
            valid_before: row.try_get("valid_before")?,
        }))
    }
