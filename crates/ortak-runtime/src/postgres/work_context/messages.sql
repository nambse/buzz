...
LEFT JOIN LATERAL (
    ...
    WHERE b.company_id=$6 AND b.public_key=ev.pubkey ORDER BY b.employee_id LIMIT 1
) author ON true
WHERE ev.community_id=$1 AND ev.channel_id=$2 AND ev.kind IN(9,40002)
  AND ev.deleted_at IS NULL AND ev.received_at <= $3
  AND (encode(ev.id,'hex')=$5 OR encode(tm.root_event_id,'hex')=$5)
ORDER BY CASE WHEN encode(ev.id,'hex')=$4 THEN 0 WHEN encode(ev.id,'hex')=$5 THEN 1
              WHEN encode(ev.id,'hex')=$8 THEN 2 ELSE 3 END,
