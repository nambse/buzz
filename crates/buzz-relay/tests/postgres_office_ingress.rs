...
async fn capture_channel(pool: &PgPool, community: CommunityId, company: Uuid, channel: Uuid) {
    use ortak_control::ports::CompanyDirectory;
    sqlx::query("INSERT INTO channels(community_id,id,name,created_by) VALUES ($1,$2,$3,$4)")
        .bind(community.as_uuid())
        .bind(channel)
        .bind(format!("capture-{channel}"))
        .bind([7u8; 32].as_slice())
        .execute(pool)
        .await
        .expect("canonical capture channel");
    sqlx::query("INSERT INTO employees(company_id,id) VALUES ($1,'fixture-capture')")
        .bind(company)
        .execute(pool)
        .await
        .expect("draft fixture employee");
    let control = ortak_control::PgControlPlane::new(pool.clone());
    let scope = control
        .resolve_company_for_community(*community.as_uuid())
        .await
        .expect("scope");
    control
        .begin_routing_capture(
            &scope,
            &[channel],
            &[serde_json::from_str("\"fixture-capture\"").expect("fixture id")],
        )
        .await
        .expect("explicit selected capture");
}
...
async fn disabled_path_keeps_inherited_persistence_without_inbox_row() {
    ...
    capture_channel(&pool, community, company_id, channel_id).await;
