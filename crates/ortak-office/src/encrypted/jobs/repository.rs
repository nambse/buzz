...
use super::{
    key, selection::lock_selection, DecryptFailure, DmClaimIdentity, DmDecryptClaim, DmJobError,
    DmOuterSource,
};
...
impl PgDecryptJobs {
    ...
    pub async fn enqueue(
        ...
        scope: &CompanyScope,
        selection: Uuid,
        source: &DmOuterSource,
    ) -> Result<bool, DmJobError> {
        ...
        let inserted: Option<Vec<u8>> = sqlx::query_scalar(include_str!("enqueue.sql"))
            ...
            .bind(selection)
            .bind(source.id.to_bytes().as_slice())
            .bind(source.created_at)
            .bind(office.generation())
            .bind(office.valid_before())
    ...
    pub async fn claim_next(
        ...
        scope: &CompanyScope,
        worker: Uuid,
    ) -> Result<Option<DmDecryptClaim>, DmJobError> {
        if worker.is_nil() {
        ...
        let code = if row.try_get::<bool, _>("expired")? {
            Some("deadline_exceeded")
        } else if row.try_get::<i32, _>("attempts")? >= 3 {
            Some("attempts_exhausted")
        } else if !row.try_get::<bool, _>("current")? {
    ...
    pub async fn record_verified(
        ...
        scope: &CompanyScope,
        claim: &DmDecryptClaim,
        result: &VerifiedDmRumor,
    ) -> Result<(), DmJobError> {
        same_scope(scope, claim)?;
        let source = result.source();
        if source.outer_id() != claim.expected.outer_id()
            || source.partition_at() != claim.expected.partition_at()
            || source.outer_author() != claim.expected.outer_author()
            || source.human() != claim.expected.human()
            || source.recipient() != claim.expected.recipient()
            || result.outer_hash() != &claim.outer_hash
        {
            return Err(DmJobError::Refused);
        }
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn lock_verified_on(
        ...
        sqlx::query(
            "SELECT event_id FROM office_inbox WHERE company_id=$1 AND event_id=$2 FOR SHARE",
        )
        .bind(scope.company_id())
        .bind(claim.expected.outer_id().to_bytes().as_slice())
    ...
    pub async fn retained_claim_matches_on(
        connection: &mut PgConnection,
        scope: &CompanyScope,
        claim: &DmDecryptClaim,
    ) -> Result<bool, DmJobError> {
...
fn same_scope(scope: &CompanyScope, claim: &DmDecryptClaim) -> Result<(), DmJobError> {
    if scope.company_id() != claim.identity.company_id
        || scope.community_id() != Some(claim.identity.community_id)
    {
        return Err(DmJobError::Refused);
    }
    Ok(())
...
fn from_row(row: PgRow) -> Result<DmDecryptClaim, DmJobError> {
    ...
    if outer.len() > super::super::MAX_OUTER_BYTES {
        return Err(DmJobError::Unavailable);
    }
    ...
    Ok(DmDecryptClaim {
        identity: DmClaimIdentity {
            ...
            selection_generation: row.try_get("selection_generation")?,
            ...
            employee_lifecycle_epoch: row.try_get("employee_lifecycle_epoch")?,
            office_generation: row.try_get("office_generation")?,
            office_binding_id: row.try_get("office_binding_id")?,
            key_version: row.try_get("key_version")?,
            decrypt_ref: CredentialRef::parse(row.try_get::<String, _>("decrypt_ref")?)
                .map_err(|_| DmJobError::Unavailable)?,
        },
        expected: ExpectedEnvelope::new(
            EventId::from_slice(&source).map_err(|_| DmJobError::Unavailable)?,
            key(&author)?,
            row.try_get("source_created_at")?,
            key(&human)?,
            key(&employee)?,
        )
        .map_err(|_| DmJobError::Unavailable)?,
        outer,
        outer_hash: hash.try_into().map_err(|_| DmJobError::Unavailable)?,
        generation: row.try_get("claim_generation")?,
        token: row.try_get("claim_token")?,
        worker: row.try_get("worker_id")?,
        crypto_deadline: row.try_get("crypto_deadline")?,
        expires_at: row.try_get("claim_expires_at")?,
    })
}
