use ortak_control::{
    postgres::{direct_channel_on, lock_office_authority_on},
    CompanyScope,
};
...
impl PgDecryptJobs {
    ...
    pub async fn register_pair(
    ...
    ) -> Result<i64, DmJobError> {
        let community = scope.community_id().ok_or(DmJobError::Refused)?;
        if pair.selection_id.is_nil()
            || pair.channel_id.is_nil()
            || pair.office_binding_id.is_nil()
            || pair.key_version < 0
            || pair.human_public_key == pair.employee_public_key
        {
        ...
        .await?
        .ok_or(DmJobError::Refused)?;
        if !direct.permits_execution()
            || direct.employee_id != pair.employee_id
...
pub(super) async fn lock_selection(
    c: &mut PgConnection,
    scope: &CompanyScope,
    id: Uuid,
) -> Result<(), DmJobError> {
