...
-- Work reference context80 authority restoration.
CREATE OR REPLACE FUNCTION ortak_run_work_context_current(company UUID, run UUID)
RETURNS BOOLEAN LANGUAGE plpgsql STABLE AS $$
...
CREATE OR REPLACE FUNCTION ortak_work_context_request80() RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION ortak_work_snapshot_admission80() RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
    PERFORM ortak_lock_office_authority(NEW.company_id);
...
DO $$ BEGIN
    IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='work_executions'::regclass AND tgname='work_context_request80') THEN
    ...
    END IF;
    IF NOT EXISTS(SELECT 1 FROM pg_trigger t JOIN pg_proc p ON p.oid=t.tgfoid
        WHERE t.tgrelid='work_executions'::regclass AND t.tgname='work_context_request80' AND t.tgtype=5
          AND t.tgdeferrable AND t.tginitdeferred AND t.tgenabled='O' AND p.proname='ortak_work_context_request80') THEN
        RAISE EXCEPTION 'Work reference80 trigger differs' USING ERRCODE='check_violation';
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='run_context_snapshots'::regclass AND tgname='work_snapshot_admission80') THEN
    ...
    END IF;
    IF NOT EXISTS(SELECT 1 FROM pg_trigger t JOIN pg_proc p ON p.oid=t.tgfoid
        WHERE t.tgrelid='run_context_snapshots'::regclass AND t.tgname='work_snapshot_admission80' AND t.tgtype=5
          AND t.tgdeferrable AND t.tginitdeferred AND t.tgenabled='O' AND p.proname='ortak_work_snapshot_admission80') THEN
        RAISE EXCEPTION 'Work reference80 trigger differs' USING ERRCODE='check_violation';
    END IF;
END $$;

DO $$ DECLARE actual JSONB; BEGIN
    SELECT jsonb_agg(jsonb_build_array('work_executions',a.attname,format_type(a.atttypid,a.atttypmod),a.attnotnull,a.attidentity,a.attgenerated,pg_get_expr(d.adbin,d.adrelid),coll.collname) ORDER BY a.attname) INTO actual FROM pg_attribute a LEFT JOIN pg_attrdef d ON d.adrelid=a.attrelid AND d.adnum=a.attnum LEFT JOIN pg_collation coll ON coll.oid=a.attcollation WHERE a.attrelid='work_executions'::regclass AND a.attname IN('context_version','reference_artifact_id') AND NOT a.attisdropped;
    IF actual IS DISTINCT FROM '[["work_executions","context_version","smallint",true,"","","0",null],["work_executions","reference_artifact_id","uuid",false,"","",null,null]]'::jsonb THEN
        RAISE EXCEPTION 'Work reference80 columns differ' USING ERRCODE='check_violation';
    END IF;
    SELECT jsonb_agg(jsonb_build_array(c.relname,k.conname,k.contype,pg_get_constraintdef(k.oid,false),k.convalidated,k.condeferrable,k.condeferred) ORDER BY c.relname,k.conname) INTO actual FROM pg_constraint k JOIN pg_class c ON c.oid=k.conrelid WHERE c.relnamespace='public'::regnamespace AND k.conname IN('work_executions_context_version_check','work_execution_reference_artifact_fk','work_execution_reference_shape','work_context_request80','work_snapshot_admission80');
    IF actual IS DISTINCT FROM '[["run_context_snapshots","work_snapshot_admission80","t","TRIGGER DEFERRABLE INITIALLY DEFERRED",true,true,true],["work_executions","work_context_request80","t","TRIGGER DEFERRABLE INITIALLY DEFERRED",true,true,true],["work_executions","work_execution_reference_artifact_fk","f","FOREIGN KEY (company_id, work_item_id, reference_artifact_id) REFERENCES artifacts(company_id, work_item_id, id)",true,false,false],["work_executions","work_execution_reference_shape","c","CHECK (((context_version = 1) OR (reference_artifact_id IS NULL)))",true,false,false],["work_executions","work_executions_context_version_check","c","CHECK ((context_version = ANY (ARRAY[0, 1])))",true,false,false]]'::jsonb THEN
        RAISE EXCEPTION 'Work reference80 constraints differ' USING ERRCODE='check_violation';
    END IF;
END $$;
